deletecondition(char collection[],char db[],char cmdword[])
{
...
i=0,j=0,k=0;
FILE *fp,*fp2;
fp2=fopen("/home/suresh/Desktop/searcer/Database/delete.txt","w");
...
else
{
	...
	*/
	...
	fclose(fp2);
	fp=fopen(path,"w");
	fp2=fopen("/home/suresh/Desktop/searcer/Database/delete.txt","r");
//char s[10];
 done=1;
	while(done)
	{
		fscanf(fp2,"%s",s);
		if(ftell(fp)<10 && ftell(fp)>1)
			fprintf(fp,"%s","\n");
		if(!strcmp(s,"end"))
			done=0;
		else if(!strcmp(s,";"))
